    ...
    ERC721TokenReceiver
{
        ...
        whenNotPaused
    {
        ...
        emit Commanded(
            contract_, fnSig_, params_, vault_, sender, address(0), msg.value
        );
    }
        ...
        whenNotPaused
    {
        ...
        emit Commanded(
            contract_, fnSig_, data_, vault_, user, address(token_), value_
        );
    }
        ...
        returns (bytes4)
    {
        ...
        nft.safeTransferFrom(
            address(this), _vault, tokenId_, safeTransferHeader()
        );
        ...
        emit Commanded(
            target, fnSig, data, _vault, from_, address(nft), tokenId_
        );
        ...
        whenNotPaused
    {
        ...
        for (uint256 i; i < length;) {
            contracts_[i].safeTransferFrom(
                sender, address(this), tokenIds_[i], data_[i]
            );
            unchecked {
                ++i;
            }
