...
contract PaymentGateway is Manager, ICommandGate, FundForwarder, ERC721TokenReceiver {
    ...
    constructor(IAuthority authority_, address[] memory vaults_) payable Manager(authority_, 0) {
    ...
    function updateTreasury(ITreasury treasury_) external onlyRole(Roles.OPERATOR_ROLE) {
        ...
        whenNotPaused
    {
        ...
        if (vault_ == vault()) {
            if (
                IWithdrawable(vault_).notifyERC20Transfer(address(token_), value_, safeTransferHeader())
        ...
        returns (bytes4)
    {
        ...
        (address target, address _vault, bytes4 fnSig, bytes memory data) =
            abi.decode(data_, (address, address, bytes4, bytes));
