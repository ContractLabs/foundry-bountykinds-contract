    ...
    AccessControlEnumerable
{
        ...
        returns (uint256)
    {
        ...
        if (IUniswapV2Pair(pool_).token1() == address(this)) {
            (nativeReserve, tokenReserve,) = IUniswapV2Pair(pool_).getReserves();
        } else {
            (tokenReserve, nativeReserve,) = IUniswapV2Pair(pool_).getReserves();
        }
        ...
        uint256 amtUSD =
            amtNative.mulDivUp(uint256(usd), 10 ** _priceFeed.decimals());

        // usd tax amount
        uint256 usdTax =
            amtUSD.mulDivUp(taxFraction(address(0)), percentageFraction());
        // native tax amount
