...
import { IMultiLevelReferralUpgradeable } from
    "./interfaces/IMultiLevelReferralUpgradeable.sol";
 ...
 * @dev When a user is referred, their referrer is set and their level is
 * updated
 * @dev Referrers are rewarded a percentage of the amount spent by their
 * referree
    ...
    IMultiLevelReferralUpgradeable
{
    ...
    /**
     * @dev Constructor that initializes the activeTimestampThreshold and
     * ratePerTier values
     * @param activeTimestampThreshold_ Timestamp threshold for an account to be
     * considered active
     * @param ratePerTier_ Percentage of amount spent by referree that is
     * rewarded to referrer for each level
     ...
     * @dev Referrer must not be the same as the referree to prevent circular
     * referrals
     * @dev If the referrer of the referrer is the same as the referree, the
     * transaction reverts
     */
        ...
        for (uint256 i; i < length;) {
            ...
            unchecked {
                if (_isAccountActiveLately(referrer)) {
                    __referrals[referrer].bonus +=
                        uint88(amount_.mulDivDown(rates[i], percentageFraction));
                }
                ++i;
            }
        }
     ...
     * @return True if the given account has been active recently, false
     * otherwise
     */
    function _isAccountActiveLately(address account_)
        ...
        returns (bool)
    {
        return block.timestamp - lastActiveTimestamp[account_]
