...
/// @author Solmate
/// (https://github.com/Rari-Capital/solmate/blob/main/src/utils/SSTORE2.sol)
...
library SSTORE2 {
    ...
    // We skip the first byte as it's a STOP opcode to ensure the contract can't
    // be called.
    ...
    function write(bytes memory data) internal returns (bytes32 ptr) {
        ...
        assembly {
             ...
             *   0x00    | 0x00                | STOP              |                               |
             * ------------------------------------------------------------------------------------+
             * @dev Prefix the bytecode with a STOP opcode to ensure it cannot
            ...
            mstore(
                ...
                or(
                    ...
                    shl(64, dataSize) // shift `dataSize` so that it lines up
                        // with the 0000 after PUSH2
                )
            )
    ...
    function writeToAddr(bytes memory data)
        internal
    ...
    function read(bytes32 ptr) internal view returns (bytes memory) {
        ...
        return readBytecode(
            pointer, DATA_OFFSET, pointer.code.length - DATA_OFFSET
        );
    }
        ...
        returns (bytes memory data)
    {
        assembly {
            ...
            // We use and(x, not(31)) as a cheaper equivalent to sub(x, mod(x,
            // 32)).
            // Adding 63 (32 + 31) to size and running the result through the
            // logic
            ...
            // Store the size of the data in the first 32 byte chunk of free
            // memory.
            mstore(data, size)

            // Copy the code into memory right after the 32 bytes we used to
            // store the size.
