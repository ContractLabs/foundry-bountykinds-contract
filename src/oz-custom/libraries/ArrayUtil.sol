...
/**
 * @title ArrayUtil
 * @dev Utility library for working with arrays. This library contains functions
...
library ArrayUtil {
    ...
    /*
    *@dev Converts an array to a set by removing duplicate elements. Uses a
    bitmap to store the seen elements for
    efficiency.
        ...
        returns (uint256[] memory)
    {
        ...
        unchecked {
            for (uint256 i; i < length; ++i) {
                ...
                //  @dev remove dupplicated element by moving it to the end of
                // the array and reduce the length, break
                // the loop only if we found an unseen element (x)
                while (
                    length > i
                        && seenBitmap.get({ value_: valI, shouldHash_: true })
                ) valI = arr_[--length];

                // @dev set seen element in the bitmap and replace dupplicated
                // one with unseen element (x)
