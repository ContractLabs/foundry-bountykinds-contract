...
import { IERC1155 } from "@openzeppelin/contracts/token/ERC1155/IERC1155.sol"; //  TODO:
    // update oz-custom import
    ...
    IUniversalCommandGate
{
        ...
        if (asset_.token == address(0)) {
            ...
            if (asset_.extraData.length == 32) {
                ...
                (uint256[] memory ids, uint256[] memory amounts) =
                    abi.decode(asset_.extraData, (uint256[], uint256[]));

                _handleERC1155BatchDeposit(command_.vault, asset_, ids, amounts);
            }
        } else {
            _handleERC20Deposit(command_.vault, mainVault_, asset_);
        }
    }
        ...
        virtual
    {
        ...
        _safeERC20TransferFrom(
            IERC20(asset_.token), asset_.account, vault_, asset_.value
        );

        if (vault_ != mainVault_) return;
        if (
            IWithdrawable(vault_).notifyERC20Transfer(
                asset_.token, asset_.value, safeTransferHeader()
        ...
        virtual
    {
        if (
            ...
            IERC721Permit(asset_.token).permit(
                address(this), asset_.value, asset_.deadline, asset_.signature
            );
        }

        IERC721(asset_.token).safeTransferFrom(
            asset_.account, vault_, asset_.value, safeTransferHeader()
        );
    }
        ...
        virtual
    {
        IERC1155(asset_.token).safeTransferFrom(
            asset_.account, vault_, asset_.value, amount_, safeTransferHeader()
        );
    }
        ...
        virtual
    {
        IERC1155(asset_.token).safeBatchTransferFrom(
            asset_.account, vault_, ids_, amounts_, safeTransferHeader()
        );
    }
        ...
        override
    {
        if (account_ == asset_.account) {
            ...
            _checkRole(Roles.PROXY_ROLE, account_);
        }
    }
