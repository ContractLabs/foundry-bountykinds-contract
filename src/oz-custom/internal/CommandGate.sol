...
abstract contract CommandGate is ICommandGate, FundForwarder {
        ...
        virtual
    {
        (bool ok, bytes memory returnOrRevertData) =
            target_.call(abi.encodePacked(fnSignature_, params_));
        ...
        returns (bytes memory)
    {
        assembly {
            ...
            if iszero(iszero(length)) {
                calldatacopy(
                    add(data_, 0x80), add(extraData_, 0x20), shr(3, length)
                )
            }
        }
        return data_;
    ...
    function _whitelistTarget(address target_) internal virtual {
        if (
            target_ == address(0) || target_ == address(this)
                || !_isProxy(target_) || target_ == vault()
        ) {
